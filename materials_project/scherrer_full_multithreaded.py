...
def process_frame(frame_idx, q, time, I, hkl_peaks, window, npz_file, save_plot_interval=20):
    frame_results = {}
    ...
    for (h, k, l, q_target) in matched_peaks:
        ...
        try:
            ...
            k_shape = 0.9
            scherrer_size = k_shape * 2 * np.pi / fwhm / 10 #/ (4 * np.pi / 3)**1/3  # in nm
            result = {
                'Frame': frame_idx,
            ...
            if frame_idx % save_plot_interval == 0:
                ...
                plt.legend()
                plt.grid()
                plot_subfolder = os.path.splitext(npz_file)[0] + '_FitPlots'
                os.makedirs(plot_subfolder, exist_ok=True)
                plot_filename = os.path.join(plot_subfolder, f'Frame{frame_idx}_HKL({h}{k}{l}).png')
...
if __name__ == "__main__":
    ...
    with ProcessPoolExecutor() as executor:
        futures = {executor.submit(process_frame, frame_idx, q, time, intensity[frame_idx, :], hkl_peaks, window, npz_file, 20): frame_idx for frame_idx in range(intensity.shape[0])}
