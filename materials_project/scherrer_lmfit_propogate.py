...
# Fit single peak using lmfit with improved constraints and re-initialization
def fit_peak(q_fit, I_fit, q_target, prev_params=None):
    ...
    default_params['sigma'].set(min=0.0005, max=0.05)
    default_params['fraction'].set(min=0, max=1)
    ...
    if prev_params is not None:
        try:
            ...
            default_params['fraction'].set(value=prev_params['fraction'])
...
if __name__ == "__main__":
    ...
    for frame_idx in tqdm(range(intensity.shape[0]), desc="Processing frames"):
        ...
        for (h, k, l, q_target) in matched_peaks:
            ...
            # Use previous fit parameters if previous R² >= 0.8, otherwise reset
            prev_params = None
            ...
            try:
                ...
                # Save fit overlay every 20 frames
                if frame_idx % 20 == 0:
    ...
    out_excel = base_output + '_LMFIT_FittingResults.xlsx'
